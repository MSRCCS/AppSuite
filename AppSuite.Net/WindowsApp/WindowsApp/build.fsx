...
let summary = "VMHub: Visual Media Hub"

// Longer description of the project
// (used as a description for NuGet package; line breaks are automatically cleaned up)
let description = "VMHub: Visual Media Hub"

// List of author names (for NuGet package)
let authors = [ "Cloud Computing & Storage Group, Microsoft Research" ]

// Tags for your project (for NuGet package)
let tags = "VMHub: Visual Media Hub"
...
let CopyBinariesFun target =
    ...
    // A customized step to copy VMHubClientExt.exe to the folder of VMHubClient, so they are co-located
    let clientFolder = "bin" @@ target @@ "Client"
    !! ( ("bin" @@ target @@ "ClientExt") + "/VMHubClientExt*" )
...
let runTests (target:string) =
    ...
    let testFiles = !! pattern
    if Seq.length testFiles > 0 then 
        testFiles
#if MONO
